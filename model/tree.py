...
##=====================================================================
# 以自己的想法实现 ret
def gen_ret(head, deprela, sent_len, directed=True):
    ret = np.zeros((sent_len, sent_len), dtype=np.float32)
    dep_matrix = np.zeros((sent_len, sent_len), dtype=np.float32)
    dep_mask = np.ones((sent_len, sent_len), dtype=np.int)

    head = head[:sent_len].tolist()
    deprela = deprela[:sent_len].tolist()
    ...
    for i in range(len(head)):
        ...
        if h != 0:
            ret[i, h-1] = 1
            dep_matrix[i, h-1] = dep
            dep_mask[i, h-1] = 0
            if directed == False:
                ret[h-1, i] = 1
                dep_matrix[h-1, i] = dep
                dep_mask[h-1, i] = 0

    return ret, dep_matrix, dep_mask
