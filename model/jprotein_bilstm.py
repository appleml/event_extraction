...
class JProtein_BiLSTM(nn.Module):
    def __init__(self, opt, bert_model, bert_tokenizer):
        super(JProtein_BiLSTM, self).__init__()
        self.opt = opt
        ...
        if self.opt['cjprot_dim']> 0:
            input_size += self.opt['cjprot_dim']
        ...
        # transformer layer
        self.encoder_layer = nn.TransformerEncoderLayer(832, 8)
    ...
    def _get_lstm_features(self, words, char_ids, pos_ids, prot_ids, cjprot_ids):
        ...
        if self.opt['cjprot_dim'] > 0:
            cjprot_embs = self.cjprot_embedding(cjprot_ids)
    ...
    def _get_transformer_features(self, words, char_ids, pos_ids, prot_ids, cjprot_ids):
        seq_lens = [len(sent.split()) for sent in words]
