...
from model.trig_bilstm import Trigger_BiLSTM

class TBiLSTM_CRF(nn.Module):
    def __init__(self, opt, bert_model, bert_tokenizer, label2id):
        super(TBiLSTM_CRF, self).__init__()
        self.opt = opt
        self.bert_model = bert_model
        self.bert_tokenizer = bert_tokenizer
        self.bilstm = Trigger_BiLSTM(self.opt, self.bert_model, self.bert_tokenizer)
        ...
        self.hidden2tag = nn.Linear(2*self.opt['rnn_hidden'], len(label2id))
        self.transformer2tag = nn.Linear(840, len(label2id))
    ...
    def neg_log_likelihood(self, words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids, tags):
        feats = self.bilstm._get_lstm_features(words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids)
        feats = self.hidden2tag(feats)
        forward_score = self._forward_alg(feats)
        gold_score = self._score_sentence(feats, tags) # 句子标注序列的打分
        return forward_score - gold_score
        # --- transformer features---------------
        # feats = self.bilstm._get_transformer_features(words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids)
        # feats = self.transformer2tag(feats)
        # forward_score = self._forward_alg(feats)
        # gold_score = self._score_sentence(feats, tags)  # 句子标注序列的打分
        # return forward_score - gold_score

    def forward(self, words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids):
        lstm_feats = self.bilstm._get_lstm_features(words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids)
        lstm_feats = self.hidden2tag(lstm_feats)
        score, tag_seq = self._viterbi_decode(lstm_feats)
        return score, tag_seq
        # --- transformer features---------------
        # transformer_feats = self.bilstm._get_transformer_features(words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids)
        # transformer_feats = self.transformer2tag(transformer_feats)
        # score, tag_seq = self._viterbi_decode(transformer_feats)
        # return score, tag_seq