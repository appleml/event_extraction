...
class Trigger_BiLSTM(nn.Module):
    def __init__(self, opt, bert_model, bert_tokenizer):
        super(Trigger_BiLSTM, self).__init__()
        self.opt = opt
        ...
        self.jprot_embedding = nn.Embedding(len(constant.JPROT_TO_ID), self.opt['jprot_dim']) if self.opt['jprot_dim'] > 0 else None
        self.cjprot_embedding = nn.Embedding(len(constant.JPROT_TO_ID), self.opt['cjprot_dim']) if self.opt['cjprot_dim'] > 0 else None
        self.ctrig_embedding = nn.Embedding(len(constant.TRIGGER_TO_ID), self.opt['ctrig_dim']) if self.opt['ctrig_dim'] > 0 else None
        ...
        if self.opt['jprot_dim'] > 0:
            input_size += self.opt['jprot_dim']
        if self.opt['ctrig_dim'] > 0:
            input_size += self.opt['ctrig_dim']
        ...
        # transformer layer
        self.encoder_layer = nn.TransformerEncoderLayer(840, 8)
    ...
    def _get_lstm_features(self, words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids):
        seq_lens = [len(sent.split()) for sent in words]
        _, word_embs = utils.get_avgword_embs(self.bert_model, self.bert_tokenizer, words, seq_lens)
        embs = [word_embs]
        if self.opt['char_dim'] > 0:
        ...
        if self.opt['jprot_dim'] > 0:
            prot_embs = self.prot_embedding(jprot_ids)
            embs.append(prot_embs)
        if self.opt['ctrig_dim'] > 0:
            ctrig_embs = self.ctrig_embedding(ctrig_ids)
    ...
    def _get_transformer_features(self, words, char_ids, pos_ids, prot_ids, jprot_ids, ctrig_ids):
        seq_lens = [len(sent.split()) for sent in words]
