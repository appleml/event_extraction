...
def read_data(data_path, genia_parse_path):
    ...
    for file_name in files:
        ...
        with open(data_path + "/" + file_name, "r") as data_fp, open(genia_parse_path + "/" + file_name, "r") as genia_parse_fp:
            ...
            multiword_trigidx = list()
            multiword_trig = list()
            multiword_event = list()
            ...
            for line in data_fp:
                ...
                if not len(line):
                    ...
                    # --------(1)删除掉四个类型的trigger(2)删除重复事件(3)删除多词trigger引发的事件(4)删除跨句子事件--------------
                    valid_event_set, multi_event_num = dns.delete_invalid_event(str_event_set, str_prot_idxs, str_event_idx, multiword_trigidx)
                    ...
                    # sent_info.prev_sentence = ""
                    # sent_info.next_sentence = ""
                    ...
                    multiword_trigidx.clear()
                    multiword_trig.clear()
                    multiword_event.clear()
                    ...
                    if trig_info[2] not in ["Entity", 'Anaphora', 'Ubiquitination', 'Protein_modification']:
                        ...
                        if len(trig_info) > 9:
                            multiword_trigidx.append(trig_info[1])
                            multiword_trig.append(line)
                    ...
                    ## 统计multi-word trigger事件的个数
                    ## % E5 Gene_expression:T22 Theme:T7
                    event_trigidx = line.split()[2].split(":")[1]
                    if event_trigidx in multiword_trigidx:
                        multiword_event_num += 1
                        multiword_event.append(line)
                else:
                    old_sent = line
