...
class Model(model.XTDS.base.Model):
    ...
    def run_train(self, train, dev, test):
        ...
        for epoch in range(self.args.train.epoch):
            self.ontology = self.ontology ## ???? the way he named is so weird
            self.train()
            ...
            iteration += iter
            summary.update({"loss": loss}) ## summary['loss']=loss ------ better
            ds = {"train": train, "dev": dev}
            ds.update(test)
    ...
    def get_info(self, x):
        ...
        for xx in x:
            ...
            cur_idx = 1
            for token in xx:
                tmp_ids = self.tokener.encode(token)[1:-1]
                #print(token, tmp_ids)
    ...
    def forward(self, texts):
        n_predicts = 3
        token_loc, token_ids, type_ids, mask_ids = self.get_info([self.cross_list(x) for x in texts])
        h, utt = self.bert(token_ids, type_ids, mask_ids)
        pred_slot = []
        for idx, locs in enumerate(token_loc):
            pred_slot.append(h[idx][locs])
        pred_slot = torch.cat(pred_slot, dim=0)
        pred_slot = self.sw(pred_slot)
        pred_intent = self.iw(utt)
        ...
        if n_predicts is None:
            ...
            intent_index_rearrange = []
            loc_i = 0
            for idx, i in enumerate(pred_intent):
                oo = pred_slot[loc_i: loc_i + len(token_loc[idx])]
                loc_i += len(token_loc[idx])
                intent_index_rearrange.append(intent_index)
